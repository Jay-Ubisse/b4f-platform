...
export function NewLocationSheet({ countryId }: { countryId: string }) {
  const [isOpen, setIsOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const t = useTranslations("Dashboard.CountryDetailsPage.CreateLocationSheet");
  ...
  return (
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      <SheetTrigger asChild>
        <Button
          className="
            w-full sm:w-auto h-11 rounded-xl px-4
        ...
        >
          ...
          <span className="font-medium">{t("CreateLocationButton")}</span>
        </Button>
      </SheetTrigger>
