...
export function CreateModuleDialog() {
  ...
  async function onSubmit(values: FormData) {
    try {
      setIsLoading(true);
      toast.loading("A cadastrar módulo...", { id: "1" });

      const fileName = sanitizeFilename(values.file.name);
      const filePath = `modules-banners/${Date.now()}_${fileName}`;
      ...
      if (response.status === 201) {
        toast.success("Módulo cadastrado com sucesso", { id: "1" });
        setOpen(false);
        form.reset();
        setIsLoading(false);
      } else {
        toast.error("Ocorreu um erro ao cadastrar módulo", { id: "1" });
        setIsLoading(false);
      }
    } catch (error) {
      console.log(error);
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="Insira o nome do módulo" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
